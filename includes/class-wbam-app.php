    ...
    public static function shortcode(): string {
        if (!is_user_logged_in()) {
            return '<div style="max-width:360px;margin:15vh auto;text-align:center;font-family:sans-serif">'
                 . '<h2>WBAM Hub</h2><p><a class="button" style="display:inline-block;padding:12px 28px;background:#111827;color:#fff;border-radius:8px;text-decoration:none" href="'
                 . esc_url(wp_login_url(home_url('/app/'))) . '">Log in</a></p></div>';
        }
        if (!current_user_can('wbam_use')) {
            return '<p style="text-align:center;margin-top:15vh">This account has no Hub access — ask a manager.</p>';
        }
        ...
        wp_localize_script('wbam-appjs', 'WBAMAPP', [
